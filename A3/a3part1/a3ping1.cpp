...
int main() 
{
    ...
    cout << "Step1. a3ping1 process starts. pid=" << getpid() << endl;
    ifstream infile;
    ...
    while(infile)
    {
        infile >> server;
        infile >> cval;
        cout << "Step3. Input Data is: " << server << ' ' << cval << endl << endl;
        ...
        if(pid1 == 0)
        {
            cout << "\tChild Process (pid=" << pid1 << ") to run: ping -c " << cval << server << endl << endl;
            close(fd1[1]);
            close(STDIN_FILENO);
            dup(fd1[0]);
